...
export const useDocumentSettingsDefaults = () => {
  ...
  const fetchDefaults = useCallback(async () => {
    ...
    try {
      // Use fetch API to query the document_settings table with labels and document types
      const response = await fetch(`https://rybezhoovslkutsugzvv.supabase.co/rest/v1/document_settings?organization_id=eq.${selectedOrganization.id}&select=document_type_id,default_label_id,document_template_labels:default_label_id(*),document_types:document_type_id(*)`, {
        headers: {
      ...
      if (data && Array.isArray(data)) {
        for (const setting of data) {
          if (setting.document_template_labels && setting.document_types) {
            labelsMap[setting.document_types.name] = setting.document_template_labels;
          }
        }
      }
