...
-- Fix the ambiguous column reference errors in quote and invoice functions
...
BEGIN
  ...
  WHERE q.organization_id = org_id 
    AND (generate_quote_number_with_gaps.workspace_id IS NULL OR q.workspace_id = generate_quote_number_with_gaps.workspace_id)
...
-- Fix renumber_quotes_after_deletion function
...
BEGIN
  ...
  FOR quote_record IN 
    ...
    WHERE q.organization_id = org_id 
      AND (renumber_quotes_after_deletion.workspace_id IS NULL OR q.workspace_id = renumber_quotes_after_deletion.workspace_id)
...
-- Also fix the invoice functions that have the same issue
...
BEGIN
  ...
  WHERE i.organization_id = org_id 
    AND (generate_invoice_number_with_gaps.workspace_id IS NULL OR i.workspace_id = generate_invoice_number_with_gaps.workspace_id)
...
-- Fix renumber_invoices_after_deletion function
...
BEGIN
  ...
  FOR invoice_record IN 
    ...
    WHERE i.organization_id = org_id 
      AND (renumber_invoices_after_deletion.workspace_id IS NULL OR i.workspace_id = renumber_invoices_after_deletion.workspace_id)
      AND i.invoice_number LIKE year_part || '-%'
