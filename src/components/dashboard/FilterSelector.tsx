    ...
    <div className="flex items-center space-x-2">
      ...
      >
        ...
        <SelectContent className="w-64 max-h-80">
          ...
          <SelectItem value="all" className="py-2 pl-12 pr-8 relative">
            <div className="flex items-center w-full">
              <span className="text-sm font-medium">Alle gegevens</span>
            </div>
            <Star 
              className={`h-3.5 w-3.5 cursor-pointer hover:text-yellow-500 transition-colors absolute right-2 top-1/2 transform -translate-y-1/2 ${favoriteFilter === 'all' ? 'fill-current text-yellow-500' : 'text-gray-400'}`}
          ...
          {groupedOptions.map((group) => (
            <div key={group.organization.id}>
              {/* Organization option */}
              <SelectItem value={`org:${group.organization.id}`} className="py-2 pl-12 pr-8 border-t border-border relative">
                ...
                <Star 
                  className={`h-3.5 w-3.5 cursor-pointer hover:text-yellow-500 transition-colors absolute right-2 top-1/2 transform -translate-y-1/2 ${favoriteFilter === `org:${group.organization.id}` ? 'fill-current text-yellow-500' : 'text-gray-400'}`}
              ...
              {group.workspaces.map((workspace) => (
                <SelectItem key={workspace.id} value={`workspace:${workspace.id}`} className="py-2 pl-14 pr-8 relative">
                  <div className="flex items-center gap-2 w-full">
                    <Briefcase className="h-3.5 w-3.5 text-muted-foreground flex-shrink-0" />
                    <span className="text-sm truncate">{workspace.name}</span>
                  </div>
                  <Star 
                    className={`h-3.5 w-3.5 cursor-pointer hover:text-yellow-500 transition-colors absolute right-2 top-1/2 transform -translate-y-1/2 ${favoriteFilter === `workspace:${workspace.id}` ? 'fill-current text-yellow-500' : 'text-gray-400'}`}
