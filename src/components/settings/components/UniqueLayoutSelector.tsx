...
export const UniqueLayoutSelector = ({ 
        ...
        <div className="absolute top-0 left-0 right-0 p-2">
          {layout.preview.headerPattern === 'gradient-header' && (
            <div className="bg-gradient-to-r from-blue-600 to-blue-500 h-6 rounded-sm flex items-center px-2">
              <div className="w-3 h-2 bg-white/80 rounded-sm mr-2"></div>
              <div className="text-xs text-white font-medium">{layout.name}</div>
            </div>
          )}
          
          {layout.preview.headerPattern === 'centered-formal' && (
            <div className="border-2 border-gray-700 bg-white p-2 text-center">
              <div className="text-xs font-serif font-bold text-gray-800">{layout.name}</div>
          ...
          {layout.preview.headerPattern === 'clean-lines' && (
            <div className="bg-white border-b border-gray-200 p-1">
              <div className="text-xs font-light text-black">{layout.name}</div>
            </div>
          )}
          ...
          {layout.preview.headerPattern === 'artistic-header' && (
            <div className="bg-gradient-to-br from-purple-600 via-purple-500 to-purple-400 p-2 rounded-lg">
              <div className="text-xs text-white text-center font-medium">{layout.name}</div>
            </div>
          )}
          
          {layout.preview.headerPattern === 'business-header' && (
            <div className="border-l-4 border-green-600 bg-white pl-2 py-1">
              <div className="text-xs text-green-800 font-semibold">{layout.name}</div>
            </div>
          )}
        </div>
  ...
  return (
    <div className="space-y-4 h-full">
      ...
      <div className="grid grid-cols-2 gap-3 overflow-y-auto max-h-[calc(100vh-250px)] pr-2">
        {layouts.map((layout) => (
          <Card 
            key={layout.id}
